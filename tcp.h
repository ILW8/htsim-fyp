...
class TcpSink : public PacketSink, public DataReceiver, public Logged {
 ...
 public:
    ...
    void replace_route(const Route* newroute);
    void sl_drop_inflight_packets(simtime_picosec packet_sent_time);
 ...
 private:
    ...
    const Route* _route;
    // denotes ealiest time where a packet should be accepted, handles
    // the dropping of inflight packets after a route switch
    simtime_picosec _earliest_acceptable_packet_sent_time;  
