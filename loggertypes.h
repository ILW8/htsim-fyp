...
class Logger {
 ...
 public:
    enum EventType { QUEUE_EVENT=0, TCP_EVENT=1, TCP_STATE=2, TRAFFIC_EVENT=3, 
		     ...
		     NDP_SINK = 18, NDP_MEMORY = 19, XCP_SINK = 20, MPXCP = 21};
...
class XcpLogger  : public Logger {
 public:
    enum XcpEvent { XCP_RCV=0, XCP_RCV_FR_END=1, XCP_RCV_FR=2, XCP_RCV_DUP_FR=3, 
		    XCP_RCV_DUP=4, XCP_RCV_3DUPNOFR=5, 
		    XCP_RCV_DUP_FASTXMIT=6, XCP_TIMEOUT=7 };
    enum XcpState { XCPSTATE_CNTRL=0, XCPSTATE_SEQ=1 };
    enum XcpRecord { AVE_CWND=0 };
    enum XcpSinkRecord { RATE = 0 };
    enum XcpMemoryRecord  {MEMORY = 0};

    virtual void logXcp(XcpSrc &src, XcpEvent ev) =0;
    virtual ~XcpLogger(){};
};

class MultipathXcpLogger  : public Logger {
 public:
    enum MultipathXcpEvent {UPDATE_TIMEOUT = 0, RATE = 1};

    virtual void logMultipathXcp(MultipathXcpSrc &src, MultipathXcpEvent ev) =0;
    virtual ~MultipathXcpLogger(){};
};
