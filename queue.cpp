...
void
Queue::set_bitrate(linkspeed_bps bitrate) {
    _bitrate = bitrate;  // note: does not change serialization time
			 // of packet already being sent, only future
			 // ones.
    _ps_per_byte = (simtime_picosec)((pow(10.0, 12.0) * 8) / _bitrate);
}

void
Queue::set_bufsize(mem_b maxsize) {
    _maxsize = maxsize;
}
...
PriorityQueue::reset() {
    for (int prio = Q_HI; prio >= Q_LO; --prio) {
	_queuesize[prio] = 0;
	while(!_queue[prio].empty()) {
	    Packet* pkt = _queue[prio].back();
	    _queue[prio].pop_back();
	    pkt->free();
	}
    }
    eventlist().cancelPendingSource(*this);
