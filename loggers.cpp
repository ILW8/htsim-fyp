...
void SinkLoggerSampling::monitorMultipathXcpSink(DataReceiver* sink) {
	...
	//mjh _multipath_seq_xcp[dynamic_cast<MultipathXcpSrc*>(sink)] = 0;
	//mjh _multipath_src_xcp[dynamic_cast<MultipathXcpSrc*>(sink)] = 0;
}
...
XcpSinkLoggerSampling::XcpSinkLoggerSampling(simtime_picosec period, 
		      EventList& eventlist):
    SinkLoggerSampling(period, eventlist, Logger::XCP_SINK, XcpLogger::RATE)
{
    cout << "XcpSinkLoggerSampling(p=" << timeAsSec(period) << ") init \n";
}
 
string XcpSinkLoggerSampling::event_to_str(RawLogEvent& event) {
    stringstream ss;
    ss << fixed << setprecision(9) << event._time;
    switch(event._type) {
    case Logger::XCP_SINK:
	assert(event._ev == XcpLogger::RATE);
	ss << " Type XCP_SINK ID " << event._id << " Ev RATE"
	   << " CAck " << (uint64_t)event._val1 << " Rate " << (uint64_t)event._val3;
	// val2 seems to always be zero - maybe a bug
	break;
    default:
