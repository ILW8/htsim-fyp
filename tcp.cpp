...
void TcpSrc::replace_route(const Route* newroute) {
	...
	_old_route = _route;
	if (_old_route->refcount() > 0) _old_route->decr_refcount();
	_route = newroute;
...
TcpSink::receivePacket(Packet& pkt) {
	...
	// // handle the case where the last downlink is severed but
	// // packet still made it to the sink due to simulator implementation
	if (p->ts() < _earliest_acceptable_packet_sent_time){
		pkt.flow().logTraffic(*p, *this, TrafficLogger::PKT_DROP);
		p->free();
		_drops++;
		// cout << "dropped packet, total " << _drops << endl;
		return;
	}
...
void TcpSink::replace_route(const Route* newroute){
...
void TcpSink::sl_drop_inflight_packets(simtime_picosec packet_sent_time){
	_earliest_acceptable_packet_sent_time = packet_sent_time;
}
