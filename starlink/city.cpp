...
#else
//#define MAXDIST 1123
// no, assume 43.8 degrees from vertical
#define MAXDIST 794.61
#endif
...
void City::update_uplinks(simtime_picosec time) {
	...
	if (dist > MAXDIST) {
	    ...
	    delete(a);
		cout << timeAsSec(time) << ": city("<< degFromRad(_latitude) << ", " << degFromRad(_longitude) << ") dropped a downlink\n";
	} else {
	    // no more to remove
	    break;
	}
...
Route* City::find_route(City& dst, simtime_picosec time) {
    Route* rt = NULL;
	// cout << "find_route const.dijkstra" << endl;
    _constellation.dijkstra(*this, dst, time);
	// cout << "find_route const.find_route" << endl;
    rt = _constellation.find_route(dst);
	// cout << "find_route end" << endl;
	if (rt) {
        rt->use_refcount();
		rt->incr_refcount();
	}
