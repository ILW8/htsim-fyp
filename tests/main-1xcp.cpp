...
int main() {
    ...
    XcpSinkLoggerSampling sinkLogger(timeFromMs(10), eventlist);
    ...
    // outgoing
#define RTT1 timeFromMs(16)
#define RTT2 timeFromMs(16)
...
#define FEEDER_BUFFER memFromPkt(50000)
#define SW_BUFFER memFromPkt(50000)
#define INGRESS_LINKSPEED speedFromMbps((uint64_t)10000)
#define CORE_LINKSPEED speedFromMbps((uint64_t)10000)
    XcpQueue srcqueue(INGRESS_LINKSPEED, FEEDER_BUFFER, eventlist,NULL,1.0);
    srcqueue.setName("SrcQueue");
    logfile.writeName(srcqueue);

    XcpPacingQueue xpq(eventlist);
    ...
    swqueueback.setName("SwQueueBack");
    logfile.writeName(swqueueback);
    ...
    simtime_picosec last_time_report = eventlist.now();
    // GO!                                                                                      
    while (eventlist.doNextEvent()) {
        if (eventlist.now() - last_time_report > timeFromMs(200)) {
            last_time_report = eventlist.now();
            cout << timeAsSec(last_time_report) << endl;
        }
    }
}
